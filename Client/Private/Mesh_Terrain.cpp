...
void CMesh_Terrain::Tick(_float _fTimeDelta)
{
	...
	//{
	//	m_pGameInstance->Transform_ToWorldSpace(m_pGameInstance->Get_Transform_Matrix_Inverse(CPipeLine::D3DTS_VIEW), m_pGameInstance->Get_Transform_Matrix_Inverse(CPipeLine::D3DTS_PROJ));
	//	m_pGameInstance->Transform_ToLocalSpace(m_pTransformCom->Get_WorldMatrix());
	//	for (size_t i = 0; i < m_pModelCom->Get_NumMeshes(); i++)
	//	{
	//		if (true == (m_pModelCom->Get_VecMeshes()[i]->Compute_MousePickedPos(m_pTransformCom->Get_WorldMatrix(), &m_vPickLocalPos, &m_vPickWorldPos)))
	//		{
	//			// ¿¹
	//			int a = 10;
	//		}
...
HRESULT CMesh_Terrain::Add_Component()
{
	...
	/* Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Field9"),
