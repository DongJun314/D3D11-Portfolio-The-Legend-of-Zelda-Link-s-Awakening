...
CFlyingOctorok::CFlyingOctorok(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    : CMonster(_pDevice, _pContext)
{
}

CFlyingOctorok::CFlyingOctorok(const CFlyingOctorok& _rhs)
    : CMonster(_rhs)
{
}

HRESULT CFlyingOctorok::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CFlyingOctorok::Initialize(void* _pArg)
{
...
void CFlyingOctorok::Priority_Tick(_float _fTimeDelta)
{
    Invalidate_ColliderTransformationMatrix();
}

void CFlyingOctorok::Tick(_float _fTimeDelta)
{
...
void CFlyingOctorok::Late_Tick(_float _fTimeDelta)
{
...
void CFlyingOctorok::Collision_ToFireBall()
{
    ...
    if (Collision_ToFireBallCollider())
    {
        ...
        if (!m_bFindBefore)
        {
            ...
            m_pGameInstance->PlaySoundEx("3_Monster_Hit.wav", SOUNDTYPE::HIT, 0.5f);
            pGameA->Add_HitCount(CMap_GameA::STAGE_3);
            m_bFindBefore = true;
        }
    }
}

_bool CFlyingOctorok::Collision_ToFireBallCollider()
{
    CPlayer* pPlayer = dynamic_cast<CPlayer*>(m_pGameInstance->Get_GameObject(LEVEL_GAMEPLAY, TEXT("Layer_Player")));
    if (nullptr == pPlayer)
        return false;
...
void CFlyingOctorok::Invalidate_ColliderTransformationMatrix()
{
...
HRESULT CFlyingOctorok::Add_Component()
{
    /* Com_Shader */
    ...
    /* Com_Model */
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_FlyingOctorok"),
...
HRESULT CFlyingOctorok::Bind_ShaderResources()
{
...
CFlyingOctorok* CFlyingOctorok::Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
{
    CFlyingOctorok* pInstance = new CFlyingOctorok(_pDevice, _pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed to Created : CFlyingOctorok");
...
CGameObject* CFlyingOctorok::Clone(void* _pArg, _uint _iLevel)
{
    CFlyingOctorok* pInstance = new CFlyingOctorok(*this);

    if (FAILED(pInstance->Initialize(_pArg)))
    {
        MSG_BOX("Failed to Cloned : CFlyingOctorok");
        Safe_Release(pInstance);
    }
