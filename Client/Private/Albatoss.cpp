...
CAlbatoss::CAlbatoss(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    : CMonster(_pDevice, _pContext)
{
}

CAlbatoss::CAlbatoss(const CAlbatoss& _rhs)
    : CMonster(_rhs)
{
}
...
HRESULT CAlbatoss::Initialize(void* _pArg)
{
...
void CAlbatoss::Priority_Tick(_float _fTimeDelta)
{
    Invalidate_ColliderTransformationMatrix();
}

void CAlbatoss::Tick(_float _fTimeDelta)
{
...
void CAlbatoss::Late_Tick(_float _fTimeDelta)
{
...
void CAlbatoss::Collision_ToFireBall()
{
    ...
    if (Collision_ToFireBallCollider())
    {
        ...
        if (!m_bFindBefore)
        {
            ...
            m_pGameInstance->PlaySoundEx("3_Monster_Albatross_Dead.wav", SOUNDTYPE::HIT, 0.3f);
            pGameA->Add_HitCount(CMap_GameA::STAGE_1);
            m_bFindBefore = true;
        }
    }
}

_bool CAlbatoss::Collision_ToFireBallCollider()
{
    CPlayer* pPlayer = dynamic_cast<CPlayer*>(m_pGameInstance->Get_GameObject(LEVEL_GAMEPLAY, TEXT("Layer_Player")));
    if (nullptr == pPlayer)
        return false;
    ...
    if (nullptr == pFireBallCollider)
        return false;
   // 3_Monster_Albatross_Dead
    //3_Monster_Albatross_Rush (1)
    //3_Monster_Albatross_Wind_High
    // 
    return m_pColliderCom->Intersect(pFireBallCollider);
}

void CAlbatoss::Invalidate_ColliderTransformationMatrix()
{
...
HRESULT CAlbatoss::Add_Component()
{
    /* Com_Shader */
    ...
    /* Com_Model */
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Albatoss"),
...
HRESULT CAlbatoss::Bind_ShaderResources()
{
...
CAlbatoss* CAlbatoss::Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
{
    CAlbatoss* pInstance = new CAlbatoss(_pDevice, _pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed to Created : CAlbatoss");
...
CGameObject* CAlbatoss::Clone(void* _pArg, _uint _iLevel)
{
    CAlbatoss* pInstance = new CAlbatoss(*this);

    if (FAILED(pInstance->Initialize(_pArg)))
    {
        MSG_BOX("Failed to Cloned : CAlbatoss");
        Safe_Release(pInstance);
    }
