...
HRESULT CGraphic_Device::Ready_Graphic_Device(_In_ const GRAPHIC_DESC& _GraphicDesc, _Inout_ ID3D11Device** _ppDevice,
	_Inout_ ID3D11DeviceContext** _ppContext)
{
	...
	if (FAILED(Ready_SwapChain(_GraphicDesc.hWnd, _GraphicDesc.bIsWindowed, _GraphicDesc.iWinSizeX, _GraphicDesc.iWinSizeY)))
		return E_FAIL;
	...
	if (FAILED(Ready_DepthStencilRenderTargetView(_GraphicDesc.iWinSizeX, _GraphicDesc.iWinSizeY)))
		return E_FAIL;
...
HRESULT CGraphic_Device::Ready_SwapChain(HWND _hWnd, _bool _bIsWindowed, _uint _iWinCX, _uint _iWinCY)
{
	...
	ZeroMemory(&SwapChain, sizeof(DXGI_SWAP_CHAIN_DESC));
	{
		...
		SwapChain.Windowed = _bIsWindowed;
...
CGraphic_Device* CGraphic_Device::Create(_In_ const GRAPHIC_DESC& _GraphicDesc, _Out_ ID3D11Device** _ppDevice,
	_Out_ ID3D11DeviceContext** _ppContext)
{
	CGraphic_Device* pInstance = new CGraphic_Device();

	if (FAILED(pInstance->Ready_Graphic_Device(_GraphicDesc, _ppDevice, _ppContext)))
	{
